...
public class CompanyController {
    ...
    @ApiResponse(responseCode = "200", description = "Список компаний успешно получен",
            content = @Content(mediaType = "application/json",
                    array = @ArraySchema(schema = @Schema(implementation = CompanyReturnDto.class))))
    ...
    @Operation(summary = "Получить компанию по ID",
            description = "Возвращает детальную информацию о компании по её уникальному идентификатору.")
    @ApiResponse(responseCode = "200", description = "Компания найдена и возвращена",
            content = @Content(mediaType = "application/json",
                    schema = @Schema(implementation = CompanyReturnDto.class)))
    @ApiResponse(responseCode = "400", description = "Некорректный ID")
    @ApiResponse(responseCode = "404", description = "Компания с указанным ID не найдена")
    public CompanyReturnDto getCompany(
    ...
    public List<EmployeeDto> getEmployeesByDepartment(
            ...
            @RequestParam @NotBlank(message = "Department name cannot be empty") String departmentName) {
    ...
    public List<CompanyReturnDto> getCompaniesWithHighSalaryEmployees(
            @Parameter(description = "Минимальный порог зарплаты", required = true, example = "5000.00")
